...
@section('title', 'Yêu cầu đổi phòng #' . $roomChangeRequest->id . ' · Homi Staff')
...
<div class="card">
    ...
    <div class="info-list mt-4">
        <div class="info-item">
            ...
            <span class="value"><a href="{{ route('staff.bookings.show', $roomChangeRequest->booking_id) }}">{{ $roomChangeRequest->booking->booking_code }}</a></span>
        </div>
        <div class="info-item">
    ...
    @if ($roomChangeRequest->status === 'pending')
        <div class="quick-actions-row mt-5">
            <form method="POST" action="{{ route('staff.room-change-requests.approve', $roomChangeRequest->id) }}"
            ...
            <form method="POST" action="{{ route('staff.room-change-requests.reject', $roomChangeRequest->id) }}" class="flex items-center gap-2">
                @csrf
