...
@section('title', ($isEdit ? 'Sửa loại phòng' : 'Thêm loại phòng') . ' · Homi Nhân viên')
...
<div class="card">
    ...
    <form method="POST"
        action="{{ $isEdit ? route('staff.room-types.update', $roomType->id) : route('staff.room-types.store') }}"
        class="form-grid">
        @csrf
        @if ($isEdit)
            @method('PUT')
