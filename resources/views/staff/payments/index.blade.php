...
@section('title', 'Thanh toán · Homi Nhân viên')
...
<div class="card">
    ...
    <form method="GET" action="{{ route('staff.payments.index') }}" class="filter-bar">
        ...
        @if (array_filter($filters))
            <a href="{{ route('staff.payments.index') }}" class="btn btn-outline">Xóa lọc</a>
        @endif
    </form>
            ...
            <table>
                ...
                <tbody>
                    @foreach ($payments as $payment)
                        <tr>
                            <td>
                                <a href="{{ route('staff.bookings.show', $payment->booking_id) }}">{{ $payment->booking->booking_code }}</a>
                            </td>
                            ...
                            <td>
                                <div class="action-row">
                                    @if ($payment->booking->canMarkPaymentAsPaid())
                                        <form method="POST" action="{{ route('staff.payments.update-status', $payment->id) }}">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <a href="{{ route('staff.bookings.show', $payment->booking_id) }}" class="btn btn-outline btn-sm">Xem đơn</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
