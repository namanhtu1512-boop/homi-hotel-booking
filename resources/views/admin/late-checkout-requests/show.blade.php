...
@section('title', 'Yêu cầu trả phòng muộn #' . $lateCheckoutRequest->id . ' · Homi Admin')
...
<div class="card">
    ...
    <div class="info-list mt-4">
        <div class="info-item">
            ...
            <span class="value"><a href="{{ route('admin.bookings.show', $lateCheckoutRequest->booking_id) }}">{{ $lateCheckoutRequest->booking->booking_code }}</a></span>
        </div>
        <div class="info-item">
        ...
        @if ($lateCheckoutRequest->status !== 'pending')
            <div class="info-item">
                ...
                <span class="value">{{ $lateCheckoutRequest->handledByUser->name ?? '—' }} lúc {{ $lateCheckoutRequest->handled_at?->format('d/m/Y H:i') }}</span>
            </div>
    ...
    @if ($lateCheckoutRequest->status === 'pending')
        <div class="quick-actions-row mt-5">
            <form method="POST" action="{{ route('admin.late-checkout-requests.approve', $lateCheckoutRequest->id) }}"
            ...
            <form method="POST" action="{{ route('admin.late-checkout-requests.reject', $lateCheckoutRequest->id) }}" class="flex items-center gap-2">
                @csrf
