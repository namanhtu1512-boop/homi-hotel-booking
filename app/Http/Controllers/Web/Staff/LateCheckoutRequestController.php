...
class LateCheckoutRequestController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        return view('staff.late-checkout-requests.index', [
            'requests'  => $this->lateCheckoutRequestService->adminList($request->only('status')),
            'filters'   => $request->only('status'),
            'usage'     => $this->lateCheckoutRequestService->usageInRange($startDate, $endDate),
    ...
    public function show(int $id): View
    {
        ...
        return view('staff.late-checkout-requests.show', ['lateCheckoutRequest' => $lateCheckoutRequest]);
    }
    ...
    public function approve(int $id, Request $request): RedirectResponse
    {
        ...
        return redirect()->route('staff.late-checkout-requests.show', $id)->with('success', 'Đã duyệt yêu cầu trả phòng muộn.');
    }
    ...
    public function reject(int $id, Request $request): RedirectResponse
    {
        ...
        return redirect()->route('staff.late-checkout-requests.show', $id)->with('success', 'Đã từ chối yêu cầu trả phòng muộn.');
    }
}
